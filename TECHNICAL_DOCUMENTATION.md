...
The project demonstrates a serverless architecture using Firebase Authentication, Cloud Firestore, and Firebase Storage. The main technical risk areas are authorization hardening, replacing the hard-coded admin login, completing Firestore-backed distributor workflows, and adding Firebase security rules.
...
The application uses Firebase as a serverless backend. There is no custom Node.js, Express, PHP, or traditional backend server in the repository.
...
The app is currently an Expo project. The repository does not include Firebase Hosting configuration such as `firebase.json`, hosting targets, or deployment scripts.
...
- Development and testing are run through Expo.
...
- `BLUETAP_CONTEXT.md`: existing engineering context note
...
BlueTap follows a serverless Firebase architecture.
...
There are no Cloud Functions or custom API endpoints in the repo. All application operations are performed from the client through Firebase SDKs.
...
- No `firebase.json` or Firebase Hosting configuration is included.
...
- Add automated tests for services and role access.
...
| Serverless architecture | Firebase Auth, Firestore, Storage; no custom server |
