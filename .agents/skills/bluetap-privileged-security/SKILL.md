...
- Public UID identity separation: Human-friendly Public UIDs (`Req001`, `Dis001`, `Mgr001`, `Adm001`) generated from atomic transactions on `accountCounters/{role}` are strictly immutable display identifiers. Internal Firebase Auth UID remains the sole authoritative identity for authentication, security rules, and authorization. Public UIDs must never be used or trusted as security credentials or session tokens.
