...
- Ensure auth state readiness (`ensureAuthStateReady()` / `auth.authStateReady()`) before concluding a session is unauthenticated during async restore or cold-boot navigation. Never sign out on transient Manager network errors (500 or temporary disconnect) if valid role claims already exist.
- Session timeout guards must track activity persistently (`recordUserActivity` / `getLastActivity`) across tab visibility changes and window focus events rather than relying on brittle in-memory interval countdowns.
