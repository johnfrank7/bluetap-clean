...
- Delivery fee and distance fee system: Branches configure `baseDeliveryFee`, `includedRadiusKm`, and `outsideRadiusFeePerKm` via Admin Branches. The backend calculates authoritative delivery fees (`baseDeliveryFee + max(0, distanceKm - includedRadiusKm) * outsideRadiusFeePerKm`) and snapshots `subtotalAtOrder`, `distanceKmAtOrder`, `deliveryFeeAtOrder`, `totalAtOrder`, and `deliveryFeePolicy` on the order document. Legacy orders are unaffected by future fee adjustments.
- Requester pending order edit: Requesters may edit order items, quantities, container type, and delivery notes while the order is in `pending` or `outside_radius_pending_approval` status via `PATCH /api/requester/orders` (`action: 'edit-order'`). The backend recalculates item subtotals, delivery fees, and grand total authoritatively and appends an edit audit log to `editHistory`.
- Strict Requester Geolocation: First-time orders require device/browser GPS (`maximumAge: 0` on web, `Location.Accuracy.High` on native). Manual pin drops are suppressed (`readOnly={true}`) in Requester ordering. Subsequent orders prefill the saved default delivery location from the profile with an explicit "Update current location" button. Never enable continuous tracking.
- Map UI and Distance Visualization: Station markers render as red landmark pins with station letter 'S' (`styles.stationPinHead`). The Requester pin features a distinct blue pulsing halo ring, blue pin head, white center dot, and label badge. A straight geodesic line with a centered Haversine distance badge connects the requester to the selected station, avoiding misleading representations of driving directions.
- Distributor Mini Map: Assigned Distributors can view a compact, read-only `LocationMap` (`height={170}`) in `RequestDetailsModal.jsx` showing the customer's delivery location, the station pin, and the geodesic distance line, with graceful fallback if coordinates are unrecorded.
