...
import { ActivityIndicator, Modal, Pressable, StyleSheet, Switch, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
export default function SecuritySettingsPage() {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setSavedSettings(authoritative); setDraftSettings(authoritative); setIsError(false);
  ...
  const toggleVerification = (key) => {
    ...
    setMessage(''); setIsError(false); setDraftSettings(next);
  };
  const updateSession = (role, key, value) => setDraftSettings((current) => ({ ...current, sessionSecurity: { ...current.sessionSecurity, [role]: { ...current.sessionSecurity[role], [key]: value } } }));
  const save = async () => {
    ...
    try {
      ...
      setSavedSettings(authoritative); setDraftSettings(authoritative); setMessage('Security settings saved.'); setIsError(false);
    } catch (error) { setDraftSettings(savedSettings); setMessage(error.message); setIsError(true); }
