...
import { captureFaceImage } from './cameraCapture';
import { CHALLENGE_CONFIG, createChallengeMachine } from './nativeChallengeMachine';

export const nativeChallengeAvailable = () => !!requireOptionalNativeModule('RNMLKitFaceDetection');

export async function runNativeFaceChallenge({ camera, challenge, assertActive, showInstruction, evaluate, complete }) {
  ...
  try {
    while (Date.now() - startedAt <= CHALLENGE_CONFIG.maxChallengeMs && Date.now() < challenge.expiresAt) {
      ...
      try {
        ...
        const state = machine.update(result.faces, capturedAt);
        ...
        showInstruction(state.reason ? 'Keep one face visible and look straight at the camera' : state.stage === 'neutral' ? 'Look straight at the camera' : state.stage === 'turn' ? challenge.instruction : 'Return to looking straight at the camera');
        if (state.challengePassed) {
          ...
          finalImage = await captureFaceImage(camera());
          assertActive();
