...
export default function SessionSecurityGuard({ children, role }) {
  ...
  const scheduleIdle = React.useCallback(() => {
    ...
    const warningRemaining = Math.max(remainingMs - WARNING_MS, 0);
    warningTimer.current = setTimeout(() => setWarning(true), warningRemaining);
    idleTimer.current = setTimeout(expire, remainingMs);
  }, [expire, role]);

  const handleActivity = React.useCallback(() => {
    recordUserActivity(role);
    scheduleIdle();
  }, [role, scheduleIdle]);
  ...
  React.useEffect(() => {
    ...
    const events = ['pointerdown', 'keydown', 'touchstart', 'scroll', 'mousemove', 'click'];
    events.forEach((event) => globalThis.addEventListener?.(event, handleActivity, { passive: true }));
    if (typeof document !== 'undefined') {
      document.addEventListener('visibilitychange', handleVisibilityOrFocus);
    }
    globalThis.addEventListener?.('focus', handleVisibilityOrFocus);
    ...
    return () => {
      ...
      globalThis.removeEventListener?.('focus', handleVisibilityOrFocus);
    };
  }, [clearTimers, expire, handleActivity, role, scheduleIdle]);

  if (!['requester', 'distributor', 'manager'].includes(role)) return children;
  return (
    <View style={styles.root} onTouchStart={handleActivity}>
      {children}
      <Modal visible={warning} transparent animationType="fade" onRequestClose={handleActivity}>
        <View style={styles.backdrop}>
          <View style={styles.card}>
            <Text style={styles.title}>Your session is about to expire</Text>
            <Text style={styles.body}>For your security, BlueTap will sign you out after one minute of inactivity.</Text>
            <View style={styles.actions}>
              <TouchableOpacity onPress={expire} style={styles.secondary}>
                <Text style={styles.secondaryText}>Sign out now</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={handleActivity} style={styles.primary}>
