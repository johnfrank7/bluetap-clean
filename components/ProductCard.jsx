...
export default function ProductCard({ product, onOrder, compact = false, selected = false }) {
  ...
  return <View style={[styles.card, compact && styles.compactCard, selected && styles.selectedCard]}>
    <View style={[styles.imageSurface, compact && styles.compactImageSurface]}>
      {imageUrl ? <Image source={{ uri: imageUrl }} style={styles.image} resizeMode="contain" /> : <View style={styles.placeholder}><Image source={require('../assets/icons/bluetaplogo.png')} style={styles.placeholderLogo} resizeMode="contain" /><Text style={styles.placeholderText}>BlueTap product</Text></View>}
    </View>
