...
`server/livenessDetector.js` still reports unavailable: client observations and
...
17.0.10. No existing face detector, expo-dev-client, eas.json, or generated native
project was found. The start scripts use Expo CLI; that does not establish which
runtime is installed on the user's phone.
...
SDK, or `npx expo run:ios --device` on macOS with Xcode/signing. Set the application's
native package/bundle identifiers when configuring those builds. EAS/custom dev
client setup is optional and has not been provisioned here. A Vercel redeploy
cannot install a native module on a phone; native binaries must be rebuilt.
...
53 Node tests pass, including head-turn direction, complete return, spikes, face
...
server/registrationFace.js (challenge response type only),
server/nativeChallengeMachine.test.js, and face-verification documentation.
